...
Scenario: Verify Pick Order in AssignedUser state can be transferred To a different user
	...
	And I Select status as 'Assigned' from Status dropdown in User Activity page
	...
	And I select transfer user as 'UserOne' from Transfer user dropdown in Transfer User popup in User Activity Page
	When I click on 'Confirm' button in Transfer User popup in User Activity Page
	Then I verify the UserCode of the activity is displayed as 'UserOne' in User Activity page